...
class NDArray:
    ...
    def make(shape, strides=None, device=None, handle=None, offset=0):
        ...
        if handle is None:
            ...
            elif device.name == "cpu":
                array._handle = array.device.cpuArray(prod(shape))
            elif device.name == "cuda":
                array._handle = array.device.gpuArray(prod(shape))
        else:
    ...
    def reshape(self, new_shape):
        ...
        new_array = NDArray.make(shape=new_shape, strides=new_strides, device=self.device, handle=self._handle, offset=self._offset)
        return new_array
        # return self
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        new_array = NDArray.make(shape=tuple(new_shape), strides=tuple(new_strides), device=self.device, handle=self._handle, offset=self._offset)
        return new_array
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert len(new_shape) == len(self._shape), "len(new_shape) != len(self._shape)"
        new_strides = []
        for i in range(len(new_shape)):
          if new_shape[i] == self._shape[i]:
            new_strides.append(self._strides[i])
          else:
            assert self._shape[i] == 1, "new_shape[i] != shape[i] for all i where shape[i] != 1"
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        new_offset, new_shape, new_strides = 0, [], []
        for i in range(len(idxs)):
          new_offset += idxs[i].start * self._strides[i]
          new_shape.append((idxs[i].stop - idxs[i].start - 1) // idxs[i].step + 1)
          new_strides.append(idxs[i].step * self._strides[i])
        new_array = NDArray.make(shape=tuple(new_shape), strides=tuple(new_strides), device=self.device, handle=self._handle, offset=new_offset)
        return new_array
        ### END YOUR SOLUTION
    ...
    def __matmul__(self, other):
        ...
        assert self.ndim == 2 and other.ndim == 2, "Matrix multiplication requires 2D arrays"
        assert self.shape[-1] == other.shape[-2], "Matrix multiplication requires inner dimensions to match"
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert len(self.shape) == len(axes), "Axes length does not match shape length"
        new_shape, idxs = [], []
        for i, (l, r) in enumerate(axes):
            new_shape.append(l + self.shape[i] + r)
            idxs.append(slice(l, l + self.shape[i], 1))
        new_array = full(tuple(new_shape), 0.0, device=self.device)
        new_array.__setitem__(tuple(idxs), self)
        return new_array
        ### END YOUR SOLUTION
...
# MODIFICATION: specify the type of parameters(NDArray)
#               and add some other basic operations
...
def reshape(array: NDArray, new_shape):
    # it's very important to compact the uncompacted array before reshape
    # this is how reshape in pytorch works
    if array.is_compact() == False:
        array = array.compact()
...
def sum(a: NDArray, axis=None, keepdims=False):
    ...
    if isinstance(axis, int):
        axis = (axis,)
    axes_list = list(axis)
    axes_list.sort(reverse=True)
    for i in axes_list:
        a = a.sum(axis=(i,), keepdims=keepdims)
    return a


def max(a: NDArray, axis=None, keepdims=False):
    if axis is None:
        return a.max(axis=None, keepdims=keepdims)
    if isinstance(axis, int):
        axis = (axis,)
    axes_list = list(axis)
    axes_list.sort(reverse=True)
    for i in axes_list:
        a = a.max(axis=(i,), keepdims=keepdims)
    return a


def flip(a: NDArray, axes):
    return a.flip(axes)


def swapaxes(a: NDArray, axis0, axis1):
    new_axes = []
    for i in range(len(a.shape)):
        new_axes.append(axis1 if i == axis0 else axis0 if i == axis1 else i)
...
def bmm(a: NDArray, b: NDArray):
    # only support [M, N] * [N, P] or [B, M, N] * [N, P]
    if a.ndim == 2 and b.ndim == 2:
        return a @ b
    assert a.ndim == 3 and b.ndim == 2 and a.shape[-1] == b.shape[-2], "matmul: shape mismatch"
    out_shape = tuple([a.shape[0], a.shape[1], b.shape[1]])
    out = NDArray.make(out_shape, device=a.device)
    for i in range(a.shape[0]):
        out[i:i + 1,:,:] = (a[i:i + 1,:,:].compact().reshape((a.shape[1], a.shape[2])) @ b) \
        .reshape((1, a.shape[1], b.shape[1]))
    return out


def stack(array_tuple, axis):
  assert len(array_tuple) > 0, "Tensor tuple can not be NULL"
  assert axis >= 0 and axis <= len(array_tuple[0].shape), "axis out of range"
  out_shape, idxs = [], []
  array_shape = array_tuple[0].shape
  for i in range(len(array_shape) + 1):
    if i < axis:
      out_shape.append(array_shape[i])
      idxs.append(slice(0, array_shape[i], 1))
    elif i == axis:
      out_shape.append(len(array_tuple))
      idxs.append(slice(0, 1, 1))
    else:
      out_shape.append(array_shape[i - 1])
      idxs.append(slice(0, array_shape[i - 1], 1))
  out = NDArray.make(tuple(out_shape), device=array_tuple[0].device)
  # out_shape[axis] = 1
  for i in range(len(array_tuple)):
    idxs[axis] = slice(i, i + 1, 1)
    # out.__setitem__(tuple(idxs), array_tuple[i].reshape(tuple(out_shape)))
    out.__setitem__(tuple(idxs), array_tuple[i])
  return out


def split(a: NDArray, axis):
  assert axis >= 0 and axis < len(a.shape), "axis out of range"
  out = []
  out_shape, idxs = [], []
  for i in range(len(a.shape)):
    if i != axis:
      out_shape.append(a.shape[i])
      idxs.append(slice(0, a.shape[i], 1))
    else:
      idxs.append(slice(0, 1, 1))
  for i in range(a.shape[axis]):
    out_array = NDArray.make(tuple(out_shape), device=a.device)
    idxs[axis] = slice(i, i + 1, 1)
    out_array = a.__getitem__(tuple(idxs)).compact().reshape(tuple(out_shape))
    out.append(out_array)
  return out


def dilate(a: NDArray, axes, dilation):
    out_shape, idxs = [], []
    for i in range(a.ndim):
        if i in axes:
            out_shape.append(a.shape[i] * (dilation + 1))
            idxs.append(slice(0, a.shape[i] * (dilation + 1), dilation + 1))
        else:
            out_shape.append(a.shape[i])
            idxs.append(slice(0, a.shape[i], 1))
    out = full(tuple(out_shape), 0.0, device=a.device)
    out.__setitem__(tuple(idxs), a)
    return out


def undilate(a: NDArray, axes, dilation):
    out_shape, idxs = [], []
    for i in range(a.ndim):
        if i in axes:
            out_shape.append(a.shape[i] // (dilation + 1))
            idxs.append(slice(0, a.shape[i], dilation + 1))
        else:
            out_shape.append(a.shape[i])
            idxs.append(slice(0, a.shape[i], 1))
    out = a.__getitem__(tuple(idxs)).compact()
    return out